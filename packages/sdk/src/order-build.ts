/**
 * Pure order-build core: deterministic appData serialization + keccak256
 * hashing, the EIP-712 order struct, and the bounded ready-to-sign order
 * builder with its amount/address guards.
 *
 * Moved verbatim from apps/mcp-server/src/ophis.ts (WP0 of the Odos extraction
 * program) so integration surfaces beyond the MCP can build byte-identical
 * orders. The MCP re-exports everything here under its original names
 * (buildOphisFullAppData as buildOphisAppData), so its call sites and tests
 * are unchanged. No network, no keys.
 *
 * NOTE: this module imports viem (keccak256 / address checksumming), declared
 * as a peerDependency of @ophis/sdk. The rest of the SDK stays dependency-free.
 */
 ...
 * orderClass, and the CIP-75 partner fee (flat `volumeBps` shape, the uniform
 * 1 bp rate via buildOphisAppDataPartnerFee) where Ophis charges one.
...
export function buildOrder(p: BuildOrderParams, nowSeconds: number): BuiltOrder {
  ...
  const receiver = ophisOrderReceiver(
    owner,
    p.unsafeCustomReceiver
      ? { unsafeCustomReceiver: checksum(p.unsafeCustomReceiver, 'unsafeCustomReceiver') }
      : {},
  );
...
// --- order guards (shared with the MCP's networked callers) ----------------

export function assertChain(chainId: number): number {
  if (!Number.isInteger(chainId) || chainId <= 0) throw new Error(`invalid chainId: ${chainId}`);
  return chainId;
}

export function checksum(addr: string, label: string): Address {
...
export function assertFeeAtoms(amount: string, label: string): void {
