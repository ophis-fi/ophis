/**
 * Conservative stablecoin registry used by server-owned Ophis integrations.
 * Keep this synchronized with the frontend's canonical STABLECOINS registry.
 * Unknown chains or tokens use the standard volatile policy and never receive
 * the reduced stable-pair improvement policy.
 */
export const OPHIS_STABLECOINS: Readonly<Record<number, ReadonlySet<string>>> = Object.freeze({
  1: new Set([
...
export function isOphisStablePair(chainId: number, sellToken: string, buyToken: string): boolean {
  const stablecoins = OPHIS_STABLECOINS[chainId];
  return (
    stablecoins?.has(sellToken.toLowerCase()) === true && stablecoins.has(buyToken.toLowerCase())
  );
}
