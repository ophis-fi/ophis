...
- **`ophisPreflight`** / **`isPreflightReady`** / **`approvalNeeded`**: one batched Multicall3 `balanceOf` + `allowance` read answering "can this order settle?" **before** the user signs. Takes any viem `PublicClient` (structural interface, no viem runtime dependency), defaults the spender to the correct per-chain vault relayer, and fails closed: an RPC failure throws, it never reports ready.
