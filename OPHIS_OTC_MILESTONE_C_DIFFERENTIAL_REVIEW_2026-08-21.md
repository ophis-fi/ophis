...
- **Low — external fork flakiness.** Throttled anonymous public RPC cold starts are not accepted as merge evidence. The browser job requires `OTC_FORK_RPC_URL`, disables Nx result reuse, and uses a fresh Anvil process with provider-aware pacing plus bounded upstream retries. PublicNode may serve unrelated host hydration, but every fork-sensitive fixture, wallet read, rendered term, preflight, send, receipt, and assertion remains on the authenticated Anvil authority.
