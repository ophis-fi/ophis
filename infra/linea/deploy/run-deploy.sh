#!/usr/bin/env bash
# Greg Phase 3c — Linea deploy runbook.
# Usage: ./run-deploy.sh linea-sepolia | linea-mainnet

set -euo pipefail
NETWORK="${1:?network arg required}"
[[ "$NETWORK" =~ ^linea-(sepolia|mainnet)$ ]] || { echo "ERROR: must be linea-sepolia | linea-mainnet" >&2; exit 1; }

REPO_ROOT="/Users/scep/greg"
ENV_FILE="$REPO_ROOT/infra/linea/.env"
...
LOG_FILE="$REPO_ROOT/infra/linea/deploy-log-${NETWORK}-$(date +%Y%m%d-%H%M%S).log"
cd "$REPO_ROOT/contracts"

echo "=== Greg Linea deploy: $NETWORK ==="
