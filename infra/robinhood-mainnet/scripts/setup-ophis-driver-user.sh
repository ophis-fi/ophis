...
echo ""
echo "PREFERRED: restore from offsite USB per docs/operations/submitter-pk-backup-runbook.md"
echo ""
echo "If entering the key by hand, use a history-safe read (the key is typed at a"
echo "prompt, not on the command line, so it never lands in shell history):"
echo "  read -rs PK; printf '%s' \"\$PK\" | sudo install -m 600 -o $USERNAME -g $(id -gn "$USERNAME" 2>/dev/null || echo staff) /dev/stdin ${CONFIG_DIR}/submitter.key; unset PK"
echo "  (paste the 0x-prefixed key at the prompt, then Enter; input is not echoed)"
