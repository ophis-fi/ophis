...
# ROBINHOOD_RPC_INTERNAL. OP_RPC_INTERNAL is likewise rejected.
if grep -qE "^[[:space:]]*(OP_RPC_INTERNAL|BASE_RPC_INTERNAL|UNICHAIN_RPC_INTERNAL)=" .env; then
  echo "ERROR: .env sets a sibling stack's bypass var — this is the Robinhood mainnet stack." >&2
  echo "       Use ROBINHOOD_RPC_INTERNAL for the Unichain bypass guard." >&2
  exit 4
fi
...
# at $HOME/.local/state/ophis/rbh-ram-pk/. ./rendered/driver.toml becomes a
...
RAM_PK_MOUNT="$HOME/.local/state/ophis/rbh-ram-pk"
RAM_PK_VOLNAME="ophis-rbh-ram-pk"
...
_mount_ram_disk_linux() {
  if mount | grep -qE " ${RAM_PK_MOUNT} type tmpfs"; then
    ...
    local marker="${RAM_PK_MOUNT}/.ophis-rbh-ram-pk-marker"
...
if ! mount_ram_disk; then
  ...
  echo "       - \$HOME/.local/state/ophis/rbh-ram-pk writable?" >&2
  echo "       - existing stale mount? (mount | grep ophis-rbh-ram-pk; diskutil unmount ...)" >&2
  exit 6
fi
...
if [[ -n "${ROBINHOOD_RPC_INTERNAL:-}" ]]; then
  if [[ "${ALLOW_RPC_BYPASS:-}" != "1" ]]; then
    ...
    echo "" >&2
    echo "    Otherwise: remove or empty the ROBINHOOD_RPC_INTERNAL line from .env." >&2
    exit 12
  fi
  echo "" >&2
  echo "*** WARNING: ROBINHOOD_RPC_INTERNAL is set + ALLOW_RPC_BYPASS=1 ***" >&2
...
# Keep erpc.yaml on RAM-disk for existing deployment mount compatibility;
# the official public voter needs no credentials.
