...
2. **Secrets**: `cp .env.example .env && chmod 600 .env`, fill the CoinGecko
   key and mandatory inter-service token. Protected eRPC reads use Cadia plus
   Robinhood's free official RPC and require no provider credential. For alerting:
