...
for tmpl in configs/*.toml.tmpl configs/*.yaml.tmpl; do
  ...
  # colima/virtiofs (this OP host) masks it via uid remapping, but we emit 0644
  # unconditionally so the exact same rendered tree also boots on the
  # native-Linux DR/VM target, and so a future `user:` pin can't silently
  # re-break it. This covers the PK-bearing driver/OKX/Odos/Enso configs too:
  # the driver + those solvers run as uid 10001 and must read their own config.
  ...
  # rendered file, and a single-file bind-mount doesn't require the container
  # to traverse that dir. Same-uid (operator) read of the RAM-disk was already
  # in-scope-accepted (Tier 1.5 docstring "Same-UID exfiltration ... NOT
  # closed"), so 0644-within-0700 widens no documented host exposure.
