...
if [[ -f "$R/render-configs.sh" ]]; then
  SB="$WORK/render-sandbox"; mkdir -p "$SB"; cp "$R/render-configs.sh" "$R/.env.example" "$SB/"
  ...
  runrender(){ ( cd "$SB" && ERPC_IMAGE="$(sed -n 's/^ERPC_IMAGE=//p' .env.example | head -1)" bash ./render-configs.sh >"$SB/out.log" 2>&1 ); echo $?; }

  printf 'POSTGRES_USER=x\nBLOCKDAEMON_OP_KEY=bd\nDRPC_API_KEY=dr\nGOLDSKY_BOOST_KEY=dr\nTENDERLY_OP_KEY=td\n' > "$SB/.env"   # neither zan name
  rc="$(runrender)"
  ...
  printf 'POSTGRES_USER=x\nZAN_API_KEY=z\nTENDERLY_OP_KEY=td\n' > "$SB/.env"   # no Boost key
  rc="$(runrender)"
  ck "no Boost key -> exits 15" "$rc" "15"
  ck "and names GOLDSKY_BOOST_KEY" "$(grep -c 'GOLDSKY_BOOST_KEY is unset/empty' "$SB/out.log")" "1"

  printf 'upstreams:\n  - id: goldsky-op\n    endpoint: https://edge.goldsky.com/boost/10?key=abc123\n' > "$SB/bdgood.yaml"
  ( cd "$SB" && bash ./render-configs.sh --check-rendered "$SB/bdgood.yaml" >/dev/null 2>&1 ); ck "query-param key present passes" "$?" "0"

  printf 'upstreams:\n  - id: goldsky-op\n    endpoint: https://edge.goldsky.com/boost/10?key=\n' > "$SB/bdempty.yaml"
