...
if [[ -r "$CURSOR" ]]; then FROM=$(( $(cat "$CURSOR") + 1 )); else FROM=$(( HEAD - FIRST_RUN_LOOKBACK )); fi
(( FROM < 0 )) && FROM=0
if (( FROM > HEAD )); then log "up-to-date (cursor $((FROM-1)) >= head $HEAD)"; exit 0; fi
TO=$(( FROM + MAX_BLOCKS - 1 )); (( TO > HEAD )) && TO=$HEAD
...
while IFS= read -r entry; do
  ...
  txfrom="$(cast tx "$txh" from --rpc-url "$RPC" 2>/dev/null || true)"
  txto="$(cast tx "$txh" to --rpc-url "$RPC" 2>/dev/null || true)"
  [[ -n "$txfrom" && "$(lc "$txfrom")" != "$SUBMITTER_LC" ]] && alert CRITICAL "settle() tx $txh sent by UNEXPECTED $txfrom (expected $SUBMITTER)"
  [[ -n "$txto"   && "$(lc "$txto")"   != "$SETTLEMENT_LC" ]] && alert WARNING  "settle() tx $txh to UNEXPECTED target $txto (expected Settlement)"
...
while IFS= read -r entry; do
  ...
  (( sell == 0 )) && continue
  bps=$(( fee * 10000 / sell ))
  (( bps > FEE_BPS_MAX )) && alert WARNING "Trade in tx $txh: fee ${bps}bps of sell (> ${FEE_BPS_MAX}bps) — possible surplus skim"
