...
#
# RUN BY: LaunchAgent ~/Library/LaunchAgents/com.ophis.op-mainnet.boot.plist
#         (RunAtLoad, one-shot — NOT KeepAlive). Logs to ~/Library/Logs/ophis-op-boot.log
#
# ⚠️ The plist hardcodes this file's absolute path; if the checkout it points at is
# moved or deleted, boot recovery silently stops working. Until 2026-07-30 this file
# was tracked ONLY on a stale WIP branch, so main had no copy at all.
#
# It deliberately does NOT run docker compose — it only starts colima. That is what
# makes a reboot safe: containers come back on their EXISTING images via
# `restart: always`, so a reboot can never silently rebuild/revert the deployed code.
# Keep it that way.
#
...
echo "==> [$(date '+%Y-%m-%dT%H:%M:%S%z')] op-boot-start done."
echo "    driver/okx-solver still need their RAM-disk key re-rendered:"
echo "      cd ${DEPLOY_DIR} && ./compose-up.sh"
echo "    (needs sudo; MUST be a clean worktree at origin/main — compose-up.sh's"
echo "     build guard refuses anything else. NOT ~/greg, which is a stale WIP tree.)"
