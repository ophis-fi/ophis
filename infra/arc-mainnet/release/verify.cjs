...
async function checkSafe(rpc, config) {
  ...
  assert.notEqual(proxyCode, '0x', 'Deploy the existing protocol Safe on Arc before the ceremony')
  // SafeProxy's executable code ends with RETURN/INVALID; only compiler metadata follows.
  // The vendored 1.3.0 artifact and 1.4.1 SafeProxy use this same dispatcher.
  ...
  // @safe-global/safe-deployments 1.37.50, assets/v{1.3.0,1.4.1}/{gnosis_safe,safe}{,_l2}.json.
  const trusted = {
