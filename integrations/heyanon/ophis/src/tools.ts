...
export const tools: AdapterExport['tools'] = [
  {
    ...
    function: {
      name: 'ophisSwap',
      description:
        'Swap one ERC-20 for another on the SAME chain via Ophis (CoW Protocol): MEV-protected and gasless. Ophis charges a 1 bp base plus capped improvement capture; the trader keeps the remainder and all improvement above the cap. The wallet signs a GPv2 order with EIP-712. Native ETH is not supported (use WETH). Not for cross-chain bridging.',
      strict: true,
      parameters: {
        type: 'object',
        properties: {
        ...
        required: ['chainName', 'sellToken', 'buyToken', 'sellAmount', 'slippageBps', 'referralCode'],
