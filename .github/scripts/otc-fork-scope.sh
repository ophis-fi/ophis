...
requires_fork() {
  [[ "${1:-}" == workflow_dispatch ]] || ! git diff --quiet HEAD^1 HEAD -- \
    apps/frontend functions tests/functions .github scripts contracts .gitmodules
}
...
if [[ "${1:-}" == --self-test ]]; then
  ...
  for path in apps/frontend/example functions/api/example tests/functions/example .github/example scripts/example contracts/foundry.toml contracts/test/otc-fork/example contracts/lib/example .gitmodules; do
