...
- Fork CI is now manual-only, with single-threaded Foundry execution. PR updates and schedules cannot consume RPC credits. The user's hold on Infura use remains in force; no private endpoint was used for this continuation.
- Local verification after a frozen dependency install: 34 OTC/navigation Jest suites (230 passed, one optional fork test skipped), 26 token-policy tests, frontend typecheck, production build, scoped lint/formatting, offline Foundry build, and the three existing gate/canary/proxy self-checks passed. A single PublicNode-backed Foundry run with retries disabled passed all seven contract invariants with zero skips in 1.44 seconds; it is separate from the outstanding authenticated CI evidence.
...
- Remaining release evidence: final-commit contract and browser fork suites using an explicitly authorized RPC budget/provider, enforceable required checks, and fresh Codex review. Production write enablement still requires separate approval.
...
- [x] **5. Establish the fork suite.** Add an isolated Foundry profile and bytecode-pinned latest-state Ethereum suite covering runtime/WETH identity, exact-approval create/fill, cancel/refund, expired deadline, competing fills, fill/cancel race, and missing approval. CI requires `OTC_FORK_RPC_URL`; the workflow and suite fail with zero skips when it is absent. The repository secret was configured on 2026-08-23; an exact-final-head run remains a Step 7 release gate.
...
- Historical configured-fork evidence: seven of seven Foundry cases and five of five earlier Cypress scenarios passed on reviewed precursor heads. The final head adds a sixth browser scenario and stronger durable recovery, so it requires a fresh configured-secret run before merge.
...
- Reproduction: start Anvil with `--fork-url <RPC> --fork-retry-backoff 1000 --retries 10 --chain-id 1`, run the frontend with `REACT_APP_OTC_WRITE_FLAG=true`, `REACT_APP_OTC_WRITE_MODE=fork`, and `REACT_APP_NETWORK_URL_1=http://localhost:8545`, then run the scoped Cypress spec with `CYPRESS_OTC_FORK_RPC_URL=http://127.0.0.1:8545`. CI requires the repository's `OTC_FORK_RPC_URL` secret and routes authenticated fork reads through a localhost-only, read-only serial proxy with bounded 429 backoff. Routine host hydration may use PublicNode, but fork fixtures, wallet reads, action terms, preflight, sends, receipts, and state assertions remain on secret-backed Anvil and are the only accepted merge-gate evidence.
- Draft PR #1228 run `32565727585` historically proved that both jobs terminate at their explicit prerequisite guards when the secret is absent instead of reporting skipped or fallback evidence. The repository secret is now configured; the live check attached to the final feature head is authoritative.
