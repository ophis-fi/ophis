...
| Spanish and Russian pages still showed critical canary warnings in English | Translate real-asset/gas, expiry, existing-order/prompt, per-order-limit and aggregate-exposure notices through Lingui. Two rendering regressions compile the actual catalogs, fail on the old notice and pass with localized expiry and amounts. |
| Runbook assumed an unwired runtime write flag could stop trading | Explicitly block activation until a real provider and witnessed flag/rollback checks exist; document the currently wired deployment/reload control and its limits. |

## Verification

- Focused Jest run: 50 suites, 412 tests pass; one optional live-network test skipped.
