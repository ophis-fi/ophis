...
| C-FINAL-02: replacement receipt accepted | viem follows replacements, including wallet cancellations. A successful self-transfer could be reported as successful OTC execution. | The shared submission boundary requires the receipt hash to match the broadcast hash. A mismatch enters durable recovery. The submission regression supplies a successful replacement receipt and expects the original hash to remain uncertain. |
...
All four findings have fixes and passing regression tests. Review fixes do not
...
Local OTC/navigation tests, frontend typecheck, scoped lint/formatting, and gate
self-tests pass. Final-head CI and authenticated Codex evidence on
[PR #1229](https://github.com/ophis-fi/ophis/pull/1229) are required before merge;
this document does not substitute for those checks. The prior local fork runs
passed all seven contract cases and all six browser scenarios without skips.
