...
| 4 | Multisig partner-fee recipient deployed and live in code | `packages/sdk/src/partner-fee.ts` + `apps/cowswap-frontend/src/ophis/partnerFeeDefault.ts` both reference `0x858f0F5e…CeF8`. 7 sdk tests pass; cowswap build green. Commit `c5f54f06f`. | PASS |
...
Engineering substrate complete. Ophis is publicly accessible at the Vercel production aliases. Partner-fee meter runs on every swap, routed to a Gnosis Safe. MEV-proof receipt downloads now include settlement tx hash + block for fulfilled orders. DCA discoverable from the home page. SVG icon ready for Safe app store. Show HN + Product Hunt posts drafted.
...
- Frontend: cowswap fork at upstream SHA `0174f35e7…`; Ophis patches tracked in `apps/frontend/.ophis-divergences.md`.
...
- CoW supported chains where Ophis's partner-fee fires today: Ethereum, BNB, Base, Arbitrum, Polygon, Avalanche, Linea, Plasma, Ink, Gnosis. Plus Sepolia for testnets.
...
- **Ophis-styled receipt PDF template.** Currently plain monospace; brand work later.
