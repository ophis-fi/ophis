...
- Immediately before the signer call, persist a unique attempt ID and the reviewed sender/target/calldata/value fingerprint with the exact canonical pending nonce sent to the wallet. Receipt reconciliation requires that exact nonce and fingerprint, including for a hash supplied from wallet activity. A missing, unrelated or older transaction never clears the lock. A stale tab cannot clear a newer attempt, even when both lack a hash.
