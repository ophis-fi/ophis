...
Ophis runs an unmodified `GPv2Settlement` + `GPv2VaultRelayer` (CoW Protocol's
audited bytecode) under a Ophis-controlled `GPv2AllowListAuthentication`. This
...
testnets below run Ophis's stack at the same Ophis-owned addresses**, which is
itself the proof — CREATE2 determinism + a constant SALT + the same Ophis
...
## Canonical Ophis addresses (live on every Ophis-target chain)
...
- **DEX:** Ophis-deployed Uniswap V2 fork (Factory + Router02), seeded WETH/USDT0 pool
...
- **DEX:** Ophis-deployed Uniswap V2 fork
...
- **Native token (surrogate):** GTETH `0x89bd2e1756ef0c73a425b0387f3a43b3b83bf755` — Linea has no canonical WETH9 predeploy on chain 59141; the in-stack bytecode-deploy attempt failed on truncated bytecode, so Ophis uses GTETH (a deployed test ERC20) as the native-token surrogate. The pool is GTETH/GTUSD instead of WETH/GTUSD.
- **DEX:** Ophis-deployed Uniswap V2 fork
...
Ophis's own orderbook on the existing 10 CoW chains so the product no
...
2. `apps/backend/crates/liquidity-sources/src/lib.rs` — empty `BaselineSource` set for Ophis-deployed chains (no upstream-blessed liquidity sources to enumerate).
...
historical reasons; serves all Ophis-target chains) overrides
`namedAccounts.{owner,manager}` to the Ophis deployer EOA on every Ophis
...
the override, the same Ophis deployer signs everything → identical CREATE2
addresses on every Ophis-target chain → CoW key-holders cannot interact
with Ophis deployments at all.
