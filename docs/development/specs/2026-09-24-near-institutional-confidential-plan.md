...
- Deployed source commit `6601207b` through the existing [production workflow](https://github.com/ophis-fi/ophis/actions/runs/36169918989), which completed successfully. Swap deployment: `9f40f428-1c0b-400e-95a6-4d1f8ced8f65`; published app bundle: `/static/app-QdINKsOP.js`. The workflow also built and deployed the explorer.
- The live bundle contains the new cache version, Arc registry entry, chain/environment address matching and ambiguity guard. Desktop Chromium on Base and mobile WebKit on Arc loaded without uncaught errors and requested CMS deployment addresses. External RPC/API traffic other than CMS was blocked during these browser smoke checks; no wallet was connected and no new funded swaps or institutional-provider writes were performed.
- The fix is pushed on `fix/solver-address-attribution-20260925`, not merged into main. Integrate it before a later main deployment overwrites the branch release.
