//! Bounded-retry helper for solver bootstrap-time RPC calls.
//!
//! Mirrors the autopilot version at
//! `apps/backend/crates/autopilot/src/util/retry.rs` and the orderbook
//! version at `apps/backend/crates/orderbook/src/retry.rs`. Duplicated to
//! avoid a cross-crate refactor for what is otherwise a hotfix; future PR
//! should consolidate all three copies into a shared crate.
//!
//! Pre-this-module, the unretrywrapped `.unwrap_or_else(...)` at
//! `infra/config/dex/file.rs:156` (authenticator contract address read)
//! caused solver containers to crash-loop every HL stack rebuild —
//! see 2026-05-18 redeploy incident in `project_ophis_roadmap.md`.
...
/// `apps/backend/crates/orderbook/src/retry.rs`. Keep all three copies
