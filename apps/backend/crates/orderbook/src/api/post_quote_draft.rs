...
/// Volume bps of the defaulted partner-fee document: the uniform 1 bp rate
/// (aligned across every tier since the 2026-08-11 cutover), parity with
/// `OPHIS_VOLUME_FEE_BPS` in `packages/sdk/src/partner-fee.ts`. Must clear the
/// ingress floor (`app_data::OPHIS_NON_STABLE_FLOOR_BPS`, enforced on fees to
/// the Ophis recipient), which a test below pins.
...
mod tests {
    ...
    fn default_app_data_is_the_sdk_cip75_document_byte_for_byte() {
        ...
        // checksummed recipient, the uniform 1 bp rate): parity with
    ...
    fn default_app_data_passes_the_backends_own_validator_and_fee_floor() {
        ...
        // fees to the Ophis recipient (1 bp == the floor), or defaulted drafts
        // would build orders the orderbook itself rejects.
