...
describe('parseQuoteRequest', () => {
  ...
  it('hard-rejects a non-zero referralFee with PARTNER_FEE_UNAVAILABLE', () => {
  ...
  it('maps referralCode integers to odos<code>', () => {
    ...
    expect(errCode(() => parseQuoteRequest({ ...golden(), referralCode: 1.5 }))).toBe('INVALID_REQUEST');
    expect(errCode(() => parseQuoteRequest({ ...golden(), referralCode: -1 }))).toBe('INVALID_REQUEST');
  });
  ...
  it('warns on ignored routing knobs and reserved features', () => {
  ...
  it('rejects malformed addresses, amounts, and same-token pairs', () => {
    ...
    expect(
      errCode(() =>
        parseQuoteRequest({ ...golden(), outputTokens: [{ tokenAddress: USDC_OP, proportion: 1 }] }),
      ),
    ).toBe('INVALID_REQUEST');
    expect(errCode(() => parseQuoteRequest({ ...golden(), userAddr: '0x123' }))).toBe('INVALID_ADDRESS');
    expect(errCode(() => parseQuoteRequest(null))).toBe('INVALID_REQUEST');
    expect(errCode(() => parseQuoteRequest({ ...golden(), chainId: 'ten' }))).toBe('INVALID_REQUEST');
  });
});
...
describe('parseSubmitRequest', () => {
  ...
  it('validates the relayed fields (public entry point, not a passthrough)', () => {
    expect(errCode(() => parseSubmitRequest({ ...base(), signature: 'beef' }))).toBe('INVALID_REQUEST');
    expect(
    ...
    expect(
      errCode(() => parseSubmitRequest({ ...base(), order: { ...order, sellTokenBalance: 'external' } })),
    ).toBe('INVALID_REQUEST');
