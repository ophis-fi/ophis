...
export async function fundForkGas(account: Address): Promise<void> {
  ...
  // re-delegated every few seconds on 2026-09-10). Code on the account makes the
  // app treat it as a smart-contract wallet and hide the EOA-only OTC write
  // path, so reset it to a plain EOA on the disposable fork before funding it.
