...
describe('getOrderVolumeFee', () => {
  it('sums every stacked Volume entry (host fee + Ophis base), ignoring price improvement', () => {
    ...
    expect(getOrderVolumeFee(fullAppData)).toBe(51)
  })

  it('reads a single-entry partnerFee like before', () => {
    const fullAppData = JSON.stringify({ appCode: 'ophis', metadata: { partnerFee: { volumeBps: 1, recipient: SAFE } } })
