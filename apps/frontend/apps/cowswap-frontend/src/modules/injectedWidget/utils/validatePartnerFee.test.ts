...
describe('validatePartnerFee()', () => {
  ...
  describe('stacking on the Ophis policy', () => {
    ...
    it.each([130, 4663])('applies the 90 BPS third-party ceiling outside the SDK enum: %s', (chainId) => {
      expect(validatePartnerFee({ bps: { [chainId]: 95 }, recipient: THIRD_PARTY })).toEqual([
        'Partner fee paid to your own address can not be more than 90 BPS: Ophis adds its own fee on top.',
      ])
      expect(validatePartnerFee({ bps: { [chainId]: 95 }, recipient: OPHIS_SAFE })).toBe(undefined)
    })
    ...
    it('keeps the plain 100 BPS ceiling for a fee paid to the Ophis Safe (the widget-react wrapper pins it)', () => {
      ...
      expect(validatePartnerFee({ bps: 101, recipient: OPHIS_SAFE })).toEqual([
        'Partner fee can not be more than 100 BPS!',
      ])
    })
  })
})
