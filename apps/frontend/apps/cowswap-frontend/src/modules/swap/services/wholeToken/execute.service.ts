...
async function validateFunding(rpc: JsonRpcProvider, quote: DirectQuote, nativeBalance: bigint): Promise<void> {
  ...
  if (
    quote.inputToken &&
    (await getInputApprovals(rpc, quote.account, quote.maxTotal, Math.floor(Date.now() / 1000) + 30)).length
  ) {
