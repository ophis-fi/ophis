...
jest.mock('./cctp.service', () => ({
  ...
  isCctpOwner: (owner: string, account: string) => owner === account,
}))
jest.mock('./cctpWallet.service', () => ({ approveCctp: jest.fn() }))

const owner = '0x0000000000000000000000000000000000000001'
const run: RunCctpAction = async (_label, action) => {
...
it('discards a delayed quote after form/account edits and does not revive it when changing back', async () => {
  ...
  const { result, rerender } = renderHook(({ key }) => useCctpQuote(owner, undefined, run, key), {
    initialProps: { key: 'old-route' },
  })
  let pending: Promise<void> | undefined
  act(() => {
    pending = result.current.loadQuote(1, 5042, '1', 'cirBTC')
  })
...
it.each(['edit', 'unmount'])('invalidates a captured signature guard on %s', async (change) => {
  ...
  jest.mocked(readCctpFunds).mockResolvedValue({ balance: 100n, allowance: 100n })
  const { result, rerender, unmount } = renderHook(({ key }) => useCctpQuote(owner, undefined, run, key), {
    initialProps: { key: 'reviewed' },
  })
  await act(async () => {
    await result.current.loadQuote(1, 5042, '1', 'cirBTC')
  })
