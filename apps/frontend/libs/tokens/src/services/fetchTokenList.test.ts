...
it('prefers HTTPS without changing the stored identity and falls back after an invalid response', async () => {
  ...
  try {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://tokens.uniswap.org', { credentials: 'omit' })
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, UNISWAP_TOKENS_LIST, { credentials: 'omit' })
  } finally {
