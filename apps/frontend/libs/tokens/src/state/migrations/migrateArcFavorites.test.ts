...
it('upgrades existing two-token Arc shortcuts once while preserving all custom choices', () => {
  ...
  const old = { 1: { custom }, [ARC_CHAIN_ID]: Object.fromEntries([ARC_USDC, ARC_EURC, custom].map((t) => [getAddressKey(t.address), t])) }
  ...
  for (const token of [ARC_USDC, ARC_EURC, ARC_CIRBTC, ARC_USYC, custom]) expect(migrated[ARC_CHAIN_ID][getAddressKey(token.address)]).toMatchObject({ address: token.address })
