  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: "Ophis uses batch auctions and a solver competition to settle every order at the best on-chain price, with MEV protection by construction. The natural-language input lets you skip token-pickers entirely. And the fee model is simple: a flat 0.10% (10 bps) fee on trade volume, applied to every trade.",
      },
    },
    {
      '@type': 'Question',
...
fee on trade volume, applied to every trade. For example, swap 1,000 USDC
and the fee is 0.10% = 1 USDC.
