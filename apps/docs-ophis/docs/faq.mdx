...
export const faqLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: "Via the CoW Protocol CIP-75 priceImprovementBps field in the order's appData, taken from the trade output at settlement. It applies on Ophis-operated chains (currently Optimism); on CoW-hosted chains Ophis adds no partner fee.",
      },
    },
    {
      '@type': 'Question',
...
ordinary trades, with only a small, capped share of any positive slippage
when a solver beats your quote. Your principal is never touched.
...
`appData`, taken from the trade output at settlement. It applies on
